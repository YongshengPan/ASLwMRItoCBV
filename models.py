...
class ResidualUNetwork(nn.Module):

    def __init__(self,  in_channels, output_channel=2, basedim=8, downdeepth=2, model_type='3D', activation_function='sigmoid', use_max_pool=False, use_triD=True, isresunet=False, use_skip=True):
        super(ResidualUNetwork, self).__init__()
        self.output_channel = output_channel
        self.model_type = model_type.lower()
        self.downdeepth = downdeepth
        self.activation_function = activation_function
    ...
    def bulid_network(self, in_channels, basedim, downdeepth=2, output_channel=2, model_type='3d'):
        self.begin_conv = nn.Sequential(self.ConvBlock(in_channels, basedim, self.extdim(3), stride=self.extdim(1), padding=self.extdim(1, 0), padding_mode='reflect'),
                                        self.InstanceNorm(basedim, affine=True),
                                        nn.ReLU())
        self.encoding_block = nn.Sequential()
        ...
        for convidx in range(0, downdeepth):
            if self.use_max_pool:
                self.encoding_block.append(
                    nn.Sequential(self.ConvBlock(basedim * 2 ** convidx, basedim * 2 ** convidx, self.extdim(3),  stride=self.extdim(1), padding=self.extdim(1, 0), padding_mode='reflect'),
                                  self.InstanceNorm(basedim * 2 ** (convidx + 1), affine=True),
                                  nn.ReLU(),
                                  self.ConvBlock(basedim * 2 ** convidx, basedim * 2 ** (convidx + 1), self.extdim(3),  stride=self.extdim(1), padding=self.extdim(1, 0), padding_mode='reflect'),
                                  self.InstanceNorm(basedim * 2 ** (convidx + 1), affine=True),
                ...
                self.encoding_block.append(
                    nn.Sequential(StackConvBlock(basedim * 2 ** convidx, basedim * 2 ** convidx, 3, 1, model_type=self.model_type, residualskip=True, ),
                                  self.ConvBlock(basedim * 2 ** convidx, basedim * 2 ** (convidx + 1), self.extdim(3), stride=self.extdim(2), padding=self.extdim(1, 0), padding_mode='reflect'),
                                  self.InstanceNorm(basedim * 2 ** (convidx + 1), affine=True),
                                  nn.ReLU()))
                # self.encoding_block.append(
                #     ResidualBlock(basedim * 2 ** convidx, basedim * 2 ** (convidx+1), 3, 2, model_type=self.model_type, change_dimension=True, ),
                #         )

        trans_dim = basedim * 2 ** downdeepth
        self.trans_block = nn.Sequential(
            StackConvBlock(trans_dim, trans_dim, 3, 1, model_type=self.model_type, residualskip=True),
            StackConvBlock(trans_dim, trans_dim, 3, 1, model_type=self.model_type, residualskip=True),
        )
        self.decoding_block = nn.Sequential()
        for convidx in range(0, downdeepth):
            if self.use_max_pool:
                ...
                self.decoding_block.append(
                    # ResidualBlock(basedim * 2 ** (convidx+2), basedim * 2 ** convidx, 3, 1, model_type=self.model_type, change_dimension=True, ),
                    nn.Sequential(
                        self.ConvBlock(basedim * 2 ** (convidx + 2), basedim * 2 ** convidx, self.extdim(3), stride=1,
                                       padding=self.extdim(1, 0), padding_mode='reflect'),
                        self.InstanceNorm(basedim * 2 ** (convidx), affine=True),
                        nn.ReLU())
                )
        self.end_conv = nn.Sequential(self.ConvBlock(basedim*2, basedim, self.extdim(3), stride=1, padding=self.extdim(1, 0), padding_mode='reflect'),
                              self.InstanceNorm(basedim, affine=True),
                              # self.ConvBlock(basedim, output_channel, self.extdim(3), stride=1, padding=self.extdim(1, 0), padding_mode='reflect')
                          )

        self.class_conv = self.ConvBlock(basedim, output_channel, self.extdim(3), stride=1, padding=self.extdim(1, 0), padding_mode='reflect')


    def bulid_network_resunet(self, in_channels, basedim, downdeepth=2, output_channel=2, model_type='3d'):
        self.begin_conv = ResidualBlock(in_channels, basedim, 3, 1, model_type=self.model_type, change_dimension=True)

        self.encoding_block = nn.ModuleList([ResidualBlock(basedim * 2 ** convidx, basedim * 2 ** (convidx + 1), 3, 2,
                                                           model_type=self.model_type, change_dimension=True) for
        ...
        self.decoding_block = nn.ModuleList([
            ResidualBlock(basedim * 2 ** (convidx + 2), basedim * 2 ** (convidx), 3, 1, model_type=self.model_type,
                          change_dimension=True) for convidx in range(0, downdeepth)
        ])

        self.class_convs = nn.ModuleList([
            self.ConvBlock(basedim * 2 ** convidx, output_channel, self.extdim(1), stride=1, padding=self.extdim(0, 0), padding_mode='reflect') for convidx in range(0, downdeepth)
        ])
        self.end_conv = ResidualBlock(basedim * 2, basedim, 3, 1, model_type=self.model_type, change_dimension=True)
        self.class_conv = self.ConvBlock(basedim, output_channel, self.extdim(3), stride=1, padding=self.extdim(1, 0), padding_mode='reflect')
    ...
    def forward(self, x, classes=None):
        ...
        for convidx in range(0, len(self.encoding_block)):
            if self.training and self.use_triD:
                ...
                # pass
            o_c1 = self.encoding_block[convidx](o_c1)
            x = F.interpolate(x, scale_factor=self.extdim(1/2), mode="nearest")
            feats.append(o_c1)
        o_c2 = self.trans_block(o_c1)
        ...
        o_c3 = self.end_conv(torch.concat((o_c2, feats[0]), dim=1))
        if classes is None:
            classes = range(self.output_channel)

        o_cls = F.conv3d(o_c3, self.class_conv.weight[classes], self.class_conv.bias[classes], padding=self.class_conv.padding, )
        # o_cls = self.class_conv(o_c3)
        prob = build_end_activation(o_cls, self.activation_function)
        # o_cls = torch.chunk(o_cls, self.output_channel, dim=1)
        return [o_cls, prob, feats]


class Generic_UNetwork(nn.Module):
    def __init__(self, in_channels, output_channel=2, basedim=8, downdeepth=2, model_type='3D', isresunet=True,
                 istransunet=False, activation_function='sigmoid', use_max_pool=False, use_attention=False, use_triD=True, use_skip=True):
        ...
        self.bulid_network(in_channels, basedim, downdeepth, output_channel)
    ...
    def bulid_network(self, in_channels, basedim, downdeepth=2, output_channel=2):
        self.begin_conv = StackConvBlock(in_channels, basedim, 3, 1, model_type=self.model_type, residualskip=self.isresunet)
        if self.use_max_pool:
            self.encoding_block = nn.ModuleList([nn.Sequential(
                self.MaxPool(self.extdim(3), self.extdim(2), padding=self.extdim(1, 0)),
        ...
        self.decoding_block = nn.ModuleList([
            StackConvBlock(basedim * 2 ** (convidx + 2), basedim * 2 ** convidx, 3, 1, model_type=self.model_type,
                           mid_channels=basedim * 2 ** (convidx + 1), residualskip=self.isresunet) for convidx in range(0, downdeepth)
        ])
        self.end_conv = StackConvBlock(basedim * 2, basedim, 3, 1, model_type=self.model_type, residualskip=self.isresunet)

        self.class_atten = nn.Sequential(
            nn.TransformerEncoder(nn.TransformerEncoderLayer(trans_dim, 8), 2),
            self.ConvBlock(trans_dim, output_channel, self.extdim(1), stride=1, padding=self.extdim(0, 0), padding_mode='reflect'))

        self.class_conv = self.ConvBlock(basedim, output_channel, self.extdim(3), stride=1, dilation=1, padding=self.extdim(1, 0), padding_mode='reflect')
    ...
    def forward(self, x, classes=None):
        ...
        if self.use_attention:
            cls_att = torch.transpose(o_c2.view([*o_c1.size()[0:2], -1]), 1, 2)
            cls_att = self.class_atten[0](cls_att)
            cls_att = torch.transpose(cls_att, 1, 2).view(o_c1.size())
            cls_att = torch.sigmoid(self.partialconv(cls_att, self.class_atten[1], classes))
            cls_att = F.max_pool3d(cls_att, cls_att.size()[2::])
        ...
        o_c3 = self.end_conv(torch.concat((o_c2, feats[0]), dim=1))
        o_cls = self.partialconv(o_c3, self.class_conv, classes, )

        # o_cls = self.class_conv(o_c3)
        prob = build_end_activation(o_cls, self.activation_function) #+ o_probs)/2
        if self.use_attention:
            prob = cls_att * prob
        return [o_cls, prob, ]

    def partialconv(self, x, conv, classes):
        if conv.weight.size(0) > len(classes):
            non_classes = [cl for cl in range(conv.weight.size(0)) if cl not in classes]
            ex_weight = conv.weight[non_classes]
            ex_bias = conv.bias[non_classes]
        else:
            ex_weight = torch.sum(conv.weight, 0, keepdim=True) * 0
            ex_bias = torch.sum(conv.bias, 0, keepdim=True) * 0 - 1e6

        if isinstance(conv, nn.Conv3d):
            o_oth = torch.max(F.conv3d(x, ex_weight, ex_bias, padding=conv.padding), dim=1, keepdim=True)[0]
            o_cls = F.conv3d(x, conv.weight[classes], conv.bias[classes], padding=conv.padding)
        elif isinstance(conv, nn.Conv2d):
            o_oth = torch.max(F.conv2d(x, ex_weight, ex_bias, padding=conv.padding), dim=1, keepdim=True)[0]
            o_cls = F.conv2d(x, conv.weight[classes], conv.bias[classes], padding=conv.padding)
        else:
            o_oth = torch.max(F.conv1d(x, ex_weight, ex_bias, padding=conv.padding), dim=1, keepdim=True)[0]
            o_cls = F.conv1d(x, conv.weight[classes], conv.bias[classes], padding=conv.padding)
        return torch.concat([o_cls, o_oth], dim=1)
